...
export const CookPaymentDetails = () => {
  ...
  useEffect(() => {
    ...
    let filteredData = allTransactions.filter(tx => {
      ...
      return selectedPeriod === 'This Month' ? txDate.getMonth() === currentDate.getMonth() : true;
    });
  ...
  return (
    <section className="flex h-screen bg-gray-50">
      ...
      <div className="md:w-4/5 p-6 overflow-auto">
        <h1 className="text-3xl font-bold text-gray-800 mb-6">Cook Payment Details</h1>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 gap-4">
              <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
                <h2 className="text-sm font-medium text-gray-500">Total Cook Payouts</h2>
                <p className="text-2xl font-bold text-gray-800">{stats.totalCookPayout}</p>
              </div>
              <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
                <h2 className="text-sm font-medium text-gray-500">Pending Payments</h2>
                <p className="text-2xl font-bold text-gray-800">{stats.pendingCookPayments}</p>
              </div>
              <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
                <h2 className="text-sm font-medium text-gray-500">Total Cooks</h2>
                <p className="text-2xl font-bold text-gray-800">{stats.cookCount}</p>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
            <div className="flex justify-between items-center mb-4">
              <h2 className="text-xl font-bold text-gray-800">Quick Transfer</h2>
              <p className="text-2xl font-bold">Rs56,772.38</p>
            </div>
            <p className="text-gray-500 text-sm mb-6">Select a cook to transfer payment</p>
              ...
              {topCooks.map(cook => (
                ...
                >
                  <div className="relative">
                    <img src={cook.image} alt={cook.name} className="w-12 h-12 rounded-full object-cover" />
                  ...
                  <span className="text-sm font-medium text-gray-900 truncate max-w-16">{cook.name}</span>
                  <span className="text-xs text-gray-500 truncate max-w-16">{cook.email.substring(0, 8)}</span>
                </div>
              ))}
            </div>
              ...
              {filteredTransactions.length > 0 ? (
                filteredTransactions.map((transaction, index) => (
                  <tr key={index}>
                    ...
                    <td className="px-6 py-4 text-sm text-gray-500">{transaction.status}</td>
                  </tr>
                ))
              ) : (
                <tr>
