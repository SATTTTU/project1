...
import { Trash2, Award, Clock, Briefcase, X } from "lucide-react"
import { useAchievementsForm } from "../formik/useAchievementFormik"
...
const AchievementsExperience = ({ userData = {} }) => {
  ...
  const { formik, isSubmitting } = useAchievementsForm(initialValues)
  ...
  const handleAddQualification = () => {
    if (newQualification.title.trim() && newQualification.institution.trim()) {
      formik.setFieldValue("qualifications", [...formik.values.qualifications, { ...newQualification }])
      setNewQualification({
        title: "",
        institution: "",
        year: new Date().getFullYear(),
        ...
        <div>
          ...
          {isEditing ? (
            ...
            <p className="text-gray-700">
              {userData?.yearsOfExperience || 0} {userData?.yearsOfExperience === 1 ? "year" : "years"}
            </p>
          )}
        </div>
          ...
          {isEditing ? (
            <div>
              <div className="flex mb-2">
                ...
                />
                <button onClick={handleAddSkill} className="bg-blue-600 text-white px-3 py-2 rounded-r-md">
                  Add
                </button>
              </div>
              ...
              <div className="flex flex-wrap gap-2 mt-2">
                {formik.values?.skills?.map((skill, index) => (
                  <div key={index} className="bg-gray-100 flex items-center px-3 py-1 rounded-full">
                    <span className="text-gray-800">{skill}</span>
                    <button onClick={() => handleRemoveSkill(skill)} className="ml-2 text-gray-500 hover:text-red-500">
                      <X className="h-3 w-3" />
                    </button>
                  </div>
                ))}
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-2 mb-2">
                ...
                <div className="flex">
                  <input
                    ...
                    placeholder="Year"
                  />
                  <button onClick={handleAddQualification} className="bg-blue-600 text-white px-3 py-2 rounded-r-md">
                    Add
                  </button>
                </div>
              </div>
