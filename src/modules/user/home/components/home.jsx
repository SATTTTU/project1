      ...
      <nav className="absolute top-0 left-0 right-0 z-50  shadow-md py-4 px-6">
        <div className="container mx-auto flex justify-between items-center">
          <Link to="/" className="flex items-center">
            ...
            <span className=" text-2xl lg:text-[#426B1F] text-[#426B1F] md:text-3xl font-bold">KhajaBox</span>
          </Link>

          <ul className="hidden md:flex space-x-6 text-[#426B1F] text-lg font-medium">
            <li><Link to="/" className="text-[#426B1F] text-xl">Home</Link></li>
          </ul>

          <button onClick={() => setIsOpen(!isOpen)} className="md:hidden text-[#426B1F]  text-2xl">
...
<p className="text-2xl mb-10 text-center flex justify-center items-center mx-auto">Login as </p>
          ...
          <Link to="/login">
            <button className="bg-[#426B1F] text-white cursor-pointer font-medium text-xl py-4 px-8 rounded-md transition-colors">
              User
            </button>
          </Link>
        </div>
