...
function getToken() {
	...
	if (userType === "admin") {
		...
		console.log(
			"Retrieved admin token from localStorage:",
		...
		console.log(
			"Retrieved cook token from localStorage:",
			token.substring(0, 10) + "..."
		);
		return token;
	}
		...
		console.log(
			"Retrieved user token from localStorage:",
			token.substring(0, 10) + "..."
		);
		return token;
	}
...
function saveUserData(userType, token) {
	...
	console.log(
		`Saving ${userType} data to localStorage with token:`,
		token.substring(0, 10) + "..."
	);
...
function authRequestInterceptor(config) {
	...
	if (!token) {
		console.warn(
			"⚠️ No valid token found in localStorage! Request might fail."
		);
	} else {
		console.log(
			"🔑 Using token from localStorage for request:",
			token.substring(0, 10) + "..."
		);
	}
